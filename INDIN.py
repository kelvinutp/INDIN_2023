...
def camera(show='n'):
    ...
    re=False
    while not re: #taking picture until getting a useful one
        re,base=cam.read()
    
    if show.lower()=='y':
        cv.imshow('imagen',base)
    
    im1=cv.GaussianBlur(base,(5,5))
    im1=cv.cvtColor(im1,cv.COLOR_BGR2GRAY)

    #finding the screw circular shape and center
    circles=cv.HoughCircles(im1,cv.HOUGH_GRADIENT,1,20,param1=50,param2=30,minRadius=0,maxRadius=0)

    # assert bool(circles) is not False,"no circles obtained"
    circles=np.uint16(np.around(circles))

    for i in circles[0,:]:
        cv.circle(base,(i[0],i[1]),i[2],(0,255,0),2) #borde del tornillo
        cv.circle(base,(i[0],i[1]),2,(0,0,255),3) #centro del tornillo

    cv.circle(base,(base.shape[1]//2,base.shape[0]//2),2,(255,0,0),3)#centro de la imagen

    #para 1 tornillo
    tornillo=circles[0,:]
    centro=np.floor_divide(base.shape,2)

    x,y,z=np.subtract(tornillo,centro)
    return x,y
